<template>
    ...
    :transform = '"translate("+[-svgConfig.width* 0.2,0]+")"'>
      ...
      <g id = 'tip' v-if = 'hover.flag && hover.type == "outer"'>
        ...
        <text fill = 'white' :dx = 'XY[0]+20 - this.svgConfig.width*0.2' :dy = 'XY[1] - 5'>Interaction: {{hover.data.data.obj}}</text>
        <text fill = 'white' :dx = 'XY[0]+20- this.svgConfig.width*0.2' :dy = 'XY[1] +15'>View: {{ hover.data.data.view.substring(hover.data.data.view.length-1,hover.data.data.view.length)}}</text>
        <text fill = 'white' :dx = 'XY[0]+20- this.svgConfig.width*0.2' :dy = 'XY[1] +35'>Time stamp: {{hover.data.data.timeStamp.toFixed(2)}}</text>
...
<script>
	...
	export default{
      ...
      outerOut(){
        ...
        console.log('nCluster',this.nCluster)
